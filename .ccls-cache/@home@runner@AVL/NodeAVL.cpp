...
#include <sstream>
#include "Prod.h"
NodeAVL::NodeAVL(int id, Prod data, NodeAVL* parent, NodeAVL* left, NodeAVL* right)
	: m_ID(id)
	, m_Data(data)
	, m_Parent(parent)
	, m_Left(left)
...
Prod NodeAVL::GetData() const
{
	return m_Data;
}

void NodeAVL::SetData(Prod data)
{
	m_Data = data;
}
